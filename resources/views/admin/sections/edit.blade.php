...
@section('title', 'Chỉnh Sửa Khu Vực - Hudson Furnishing')
@section('page-title', 'Chỉnh Sửa Khu Vực')

@section('content')
    <div class="card shadow">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-light">Chỉnh sửa khu vực: {{ $section->name }}</h6>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('admin.sections.update', $section) }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug"
                        name="slug" value="{{ old('slug', $section->slug) }}">
                    @error('slug')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
